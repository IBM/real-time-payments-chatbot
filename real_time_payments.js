...
if(	appEnv &&
	...
	//run locally
	if (process.env.CRED_REAL_TIME_PAYMENTS_URL && process.env.CRED_REAL_TIME_PAYMENTS_ACCESSTOKEN) {
		serviceBrokerUri = process.env.CRED_REAL_TIME_PAYMENTS_URL;
		serviceBrokerAccessToken = process.env.CRED_REAL_TIME_PAYMENTS_ACCESSTOKEN;
	}
...
function lookupTokenByAccountNumber(number, username, fn) {
  ...
  console.log(serviceBrokerAccessToken);
  console.log(serviceBrokerUri);
...
var real_time_payments = {
  ...
  // authenticates a sample application user
   authenticate: function(username, password, fn) {
    ...
    this.lookupDDACustomer(makeUniqueUsername(username), function(err, user){
  ...
  send: function(token, fname, lname, amount, account, user, fn) {
  ...
  listDDAAccounts: function(id, fn) {
  	var client = new Client(serviceBrokerRequestParameters);
  	var args = {
  		headers: GET_HEADERS
  	};
  ...
  createLocalUser: function(username, password, fname, lname, fn) {
  ...
  createCXCParticipant: function(username, fname, lname, fn) {
  	var client = new Client(serviceBrokerRequestParameters);
  	var args = {
  		headers: FTM_POST_HEADERS,
  		data:  '{' +
  ...
  lookupCXCParticipant: function(username, fn) {
  	var cxcUser = null;
  	var client = new Client(serviceBrokerRequestParameters);
  	var args = {
  		headers: GET_HEADERS
  ...
  listTokensUsingCXCTokens: function(username, fn) {
  	var err = null;
  	var tokens = null;
  	var client = new Client(serviceBrokerRequestParameters);
  	var args = {
  ...
  createCXCToken: function(number, user, token, fn) {
  ...
  editCXCToken: function(number, user, token, fn) {
  	var account = findAccount(number, user);
  	var tokenType = getTokenType(token);
  ...
  deleteCXCToken: function(token, id, fn) {
  ...
  listRecipientsUsingCXCRecipients: function(username, fn) {
  	var err = null;
  	var recipients = null;
  	var client = new Client(serviceBrokerRequestParameters);
  	var args = {
  ...
  createCXCRecipient: function(token, fname, lname, username, fn) {
  	if( username ) {
  		var tokenType = getTokenType(token);
  ...
  editCXCRecipient: function(fname, lname,_token, _fname, _lname, username, fn) {
  ...
  deleteCXCRecipient: function(token, fname, lname, username, fn) {
  ...
  getPayment: function(paymentID, status, user, fn) {
  	var client = new Client(serviceBrokerRequestParameters);
  	var args = {
  		headers: GET_HEADERS
  	};
  ...
  getPaymentRequest: function(paymentRequestID, username, fn) {
  	var client = new Client(serviceBrokerRequestParameters);
  	var args = {
  		headers: GET_HEADERS
  	};
  ...
  request: function(token, fname, lname, amount, account, username, fn) {
  	if( username ) {
  ...
  listPaymentRequests: function(username, fn) {
  	var client = new Client(serviceBrokerRequestParameters);
  	var args = {
  		headers: GET_HEADERS
  	};
  ...
  // invokes the BlueMix Service's REST API to lookup a DDA Customer
  lookupDDACustomer: function(id, fn) {
  	var client = new Client(serviceBrokerRequestParameters);
  	var args = {
  		headers: GET_HEADERS
  	};
...
module.exports = real_time_payments;
